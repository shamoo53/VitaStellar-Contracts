//! # Reentrancy Guard
//!
//! Minimal per-contract reentrancy lock for Soroban contracts.
//! Pure helper crate — intentionally has NO `#[contract]` / `#[contractimpl]`
//! so it is safe to depend on from multiple contract crates without causing
//! duplicate-symbol WASM linker errors (e.g. duplicate `initialize`).
#![no_std]
